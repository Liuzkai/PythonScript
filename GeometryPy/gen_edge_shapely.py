...
def main(argv):
    ...
    if len(argv) > 2:
        has_loc = True
        gen_loc = (float(argv[2]), float(argv[3]), float(argv[4]))

    filters = {'Width': 500.0, 'SplineType': 1}
    points_json = load_spline_json(ipath + '/roadmap_segment.json', **filters)  # read the roadmap json file
    ...
    '''write to json'''
    content = {}
    with open(ipath + '/output_cross_splines.json', 'w') as pf:
