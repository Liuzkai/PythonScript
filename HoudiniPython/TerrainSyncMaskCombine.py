...
def computeRange():
    ...
    for parm in parms:
        # print(parm.name())
        if 'type' in parm.name():
            if parm.evalAsInt() == 1:
                str1 = parm.name().split("_")[0]
                str2 = parm.name().split("_")[1]
                index1 = filter(str.isdigit, str1)
                index2 = filter(str.isdigit, str2)
                minlock = 'minhauto' + str(index1) + '_' + str(index2)
                maxlock = 'maxhauto' + str(index1) + '_' + str(index2)
                if hou.parm(minlock).evalAsInt() == 1:
                    minheight_parm = 'minheight' + str(index1) + '_' + str(index2)
                    hou.parm(minheight_parm).set(range[0])
                if hou.parm(maxlock).evalAsInt() == 1:
                    maxheight_parm = 'maxheight' + str(index1) + '_' + str(index2)
                    hou.parm(maxheight_parm).set(range[1])
...
def create_entity(kwargs):
    ...
    # check the children nodes nums
    children = net.children()
    parm_name = "layer_group%d" % group if isAttrib == 1 else "noise_group%d" % group
    if (len(children) - 3) != node.parm(parm_name).evalAsInt():
        initialize_subnet(net)
    ...
    for i in range(index - node_num):
        ...
        for n, pt in enumerate(entity_parms):
            ...
            if 'ramp' in sourceparm_name:
                ramp_str = "../../../../../ramp%d_%d" % (
                    group_index, entity_index) if isAttrib == 1 else "../../../../../n_ramp%d_%d" % (
                group_index, entity_index)
                pt.set(ramp_str)
                continue
...
def create_data_entity(kwargs):
    print
    "python module: create_data_entity"
    index = int(kwargs['script_value0'])
    node = kwargs['node']
    parm = kwargs['parm']
    isAttrib = 1

    # check node nums and parm
    net = node.node('Data_Group')
    children = net.children()
    if len(children) - 3 != parm.evalAsInt():
        initialize_subnet(net)
...
def initial_data_group(node):
    print
    "python module: initial_data_group"
...
def initialize_subnet(net):
    print
    "python module: initialize_subnet"
    children = net.children()
    for node in children:
        node.destroy()
    upper = net.createNode('null', 'Upper')
    original = net.createNode('null', 'Original')
    output = net.createNode('output', 'output0')
    upper.setInput(0, net.indirectInputs()[0], 0)
    upper.moveToGoodPosition()
    original.setInput(0, net.indirectInputs()[0], 0)
    original.moveToGoodPosition()
    output.setInput(0, original)
    output.moveToGoodPosition()